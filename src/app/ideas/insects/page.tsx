...
import {
  loadInsectsFromCloud,
  saveInsectsLocal,
  deleteInsectFromCloud,
  uploadPhotoToCloud,
  Insect,
} from '@/lib/feishuStore';
...
const RARITY_CONFIG: Record<Rarity, { label: string; color: string; bg: string; stars: number }> = {
  common:    { label: '普通',   color: '#6b7280', bg: 'from-gray-400 to-gray-500',   stars: 1 },
  uncommon:  { label: '稀有',   color: '#16a34a', bg: 'from-green-500 to-emerald-600', stars: 2 },
  rare:      { label: '珍稀',   color: '#2563eb', bg: 'from-blue-500 to-indigo-600',  stars: 3 },
  legendary: { label: '传说',   color: '#f59e0b', bg: 'from-amber-400 to-orange-500', stars: 3 },
};
...
function InsectCard({ insect, onClick }: { insect: Insect; onClick: () => void }) {
  ...
  return (
    ...
    >
      <div
        className={`relative bg-gradient-to-br ${cfg.bg} rounded-2xl p-3 pb-2 shadow-lg hover:scale-105 transition-transform duration-200`}
      >
        ...
        <div className={`absolute top-2 left-2 px-2 py-0.5 rounded-full text-white text-xs font-bold`} style={{ backgroundColor: cfg.color }}>
          {cfg.label}
        </div>

        <h3 className="text-white font-black text-base text-center leading-tight">{insect.name}</h3>
        <div className="flex justify-center mt-1">
          <span className={`text-xs px-2 py-0.5 rounded-full ${TYPE_COLORS[insect.type as BugType] || TYPE_COLORS['其他']}`}>
            {insect.type}
          </span>
        </div>
...
export default function InsectsPage() {
  ...
  useEffect(() => {
    loadInsectsFromCloud().then((data) => {
      if (data.length > 0) {
        setInsects(data);
        saveInsectsLocal(data);
      }
    });
  }, []);

  // Local save on change
  useEffect(() => {
    if (insects.length > 0) {
      saveInsectsLocal(insects);
    }
  }, [insects]);
  ...
  const stats = {
    ...
    legendary: insects.filter((i) => i.rarity === 'legendary').length,
  ...
  const handleDelete = async (id: string) => {
    ...
    if (insect?._recordId) {
      await deleteInsectFromCloud(insect._recordId);
    }
    const updated = insects.filter((i) => i.id !== id);
    setInsects(updated);
    saveInsectsLocal(updated);
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {[
            ...
            { label: '传说虫', value: stats.legendary, icon: '🌟', color: 'bg-amber-50' },
        ...
        <div className="flex items-center gap-2 mb-6 flex-wrap">
          ...
          {(['全部', '甲虫', '蝴蝶', '蜻蜓', '蚂蚁', '蜜蜂', '螳螂', '蝉', '萤火虫', '其他'] as const).map((t) => (
            <button key={t} onClick={() => setFilterType(t as BugType | '全部')}
              className={`text-xs px-3 py-1.5 rounded-full border transition-all ${filterType === t ? 'bg-emerald-500 text-white border-emerald-500 shadow-sm' : 'bg-white text-gray-500 border-gray-200 hover:border-emerald-300'}`}>
              {t}
            </button>
          ))}
          ...
          {(['全部', 'common', 'uncommon', 'rare', 'legendary'] as const).map((r) => (
            <button key={r} onClick={() => setFilterRarity(r as Rarity | '全部')}
              className={`text-xs px-3 py-1.5 rounded-full border transition-all ${filterRarity === r ? 'bg-emerald-500 text-white border-emerald-500 shadow-sm' : 'bg-white text-gray-500 border-gray-200 hover:border-emerald-300'}`}>
          ...
          <div className="bg-white rounded-3xl border border-gray-100 p-6 mb-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={form.type || '甲虫'} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value }))}
                  className="w-full mt-1 px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-300">
              ...
              <div>
                ...
                <input type="date" value={form.dateFound || ''}
                  onChange={(e) => setForm((f) => ({ ...f, dateFound: e.target.value }))}
                  className="w-full mt-1 px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-300" />
              </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filtered.map((insect) => (
              <InsectCard key={insect.id} insect={insect} onClick={() => setSelectedInsect(insect)} />
            ))}
          </div>
        )}
      </div>
      ...
      {selectedInsect && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl max-h-[85vh] overflow-y-auto"
            ...
            {(() => {
              const cfg = RARITY_CONFIG[selectedInsect.rarity as Rarity] || RARITY_CONFIG.common;
              return (
                <>
                  <div className={`relative bg-gradient-to-br ${cfg.bg} rounded-2xl p-6 text-white mb-4`}>
                    ...
                    <div className="text-center">
                      ...
                      <div className="flex justify-center gap-2 mt-1">
                        <span className={`text-xs px-3 py-0.5 rounded-full ${TYPE_COLORS[selectedInsect.type as BugType] || TYPE_COLORS['其他']}`}>
                          {selectedInsect.type}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-3 mt-5">
                    ...
                    <button onClick={() => setSelectedInsect(null)}
